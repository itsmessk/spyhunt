...
def run_nuclei_ssrf(target, save_file=None):
    try:
        command = f"nuclei -u {target} -t ssrf -silent"
        result = subprocess.run(command, shell=True, capture_output=True, text=True)

        if result.returncode == 0:
            output = result.stdout.strip()
            if output:
                print(f"{Fore.RED}Potential SSRF vulnerability detected on {target}:{Style.RESET_ALL}\n{output}")
                if save_file:
                    with open(save_file, 'a') as f:
                        f.write(f"{target} - {output}\n")
            else:
                print(f"{Fore.YELLOW}No SSRF vulnerabilities detected on {target}.{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}Error running Nuclei on {target}: {result.stderr}{Style.RESET_ALL}")
    except Exception as e:
        print(f"{Fore.RED}Unexpected error running Nuclei on {target}: {e}{Style.RESET_ALL}")
...
if args.ssrf:
    ...
    for domain in domains:
        ...
        run_nuclei_ssrf(domain, args.save)
